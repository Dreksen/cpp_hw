...
bool operator==(const Range& range, const std::vector<int64_t>& vec) {
    // std::cout << "SZ " << range.Size() << std::endl;
    // std::cout << "VSZ " << vec.size() << std::endl;
    // std::cout << "rng " << *range.begin() << ' ' << *range.end() << std::endl;
    if (range.Size() != static_cast<int64_t>(vec.size())) {
        return false;
    }
    auto range_iter = range.begin();
    auto vec_iter = vec.begin();
    if (vec_iter != vec.end() and range_iter != range.end())
        // std::cout << "BEGIN " << *vec_iter << ' ' << *range_iter << std::endl;
    ...
    // std::cout << "BE " << *range_iter << ' ' << *range.end() << std::endl;
